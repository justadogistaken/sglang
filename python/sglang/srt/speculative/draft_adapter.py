...
class DraftAdapterThread:
    ...
    control_recv_socket: zmq.Socket | None = (
        None  # verifier -> drafter recv control messages
    )
    result_send_sockets: dict[int, zmq.Socket] = field(
        default_factory=dict
    )  # drafter -> verifier send draft tokens
    _pending_lock: threading.Lock = field(
        default_factory=threading.Lock
    )  # used to protect _pending_controls
